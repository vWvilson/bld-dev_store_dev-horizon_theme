  ...
  function init(container) {  
    ...
    var ajaxLoadPage = function(url) {
      ...
      fetch(url).then(response => response.text()).then(data => {
        var c = document.getElementById('collection-main');
        var p = document.getElementById('pagination-page');
        var bC = document.querySelector('.breadcrumbs');
        var f = document.getElementById('CollectionFiltersForm');
        const parser = new DOMParser();
        const htmlDocument = parser.parseFromString(data, 'text/html');
        var nC = htmlDocument.documentElement.querySelector('#collection-main');
        var nP = htmlDocument.documentElement.querySelector('#pagination-page');
        var nBC = htmlDocument.documentElement.querySelector('.breadcrumbs');
        var nF = htmlDocument.documentElement.querySelector('#CollectionFiltersForm');
        if (c) {
          c.replaceWith(nC);
        }
        if (p) {
    ...
    var filters = {
      action: function(e) {
        ...
        container.querySelector('select.bootstrap-select').onchange = function(s) {
          var u = new URL(window.location);
          var params = u.searchParams;
    ...
    function PriceChange() {
      ...
      var pIN = container.querySelectorAll('.field__input[type=\"number\"]');
...
theme.CollectionSection = (function () {
  function CollectionSection(container) {
    var $container = this.$container = $(container);
    var sectionId = $container.attr('data-section-id');
    var collection = this.collection = '#collection-section-'+sectionId;
    theme.collection[collection] = new theme.Collection(container);
  }

  return CollectionSection;
}());
theme.CollectionSection.prototype = _.assignIn({}, theme.CollectionSection.prototype, {
  onUnload: function() {
    delete theme.collection[this.collection];
  },
});  
